...
func main() {
	...
	mux.HandleFunc(pat.Get("/new_items/:root_category_id.json"), getNewCategryItemsNew)
...
func getCategoryByIDNew(q sqlx.Queryer, categoryID int) (category Category, err error) {
	var rootIds = map[int]int{1: 1, 2: 1, 3: 1, 4: 1, 5: 1, 6: 1, 10: 10, 11: 10, 12: 10, 13: 10, 14: 10, 15: 10, 20: 20, 21: 20, 22: 20, 23: 20, 24: 20, 30: 30, 31: 30, 32: 30, 33: 30, 34: 30, 35: 30, 40: 40, 41: 40, 42: 40, 43: 40, 44: 40, 45: 40, 50: 50, 51: 50, 52: 50, 53: 50, 54: 50, 55: 50, 56: 50, 60: 60, 61: 60, 62: 60, 63: 60, 64: 60, 65: 60, 66: 60}
	var rootCategoryNames = []string{"ソファー", "家庭用チェア", "キッズチェア", "オフィスチェア", "折りたたみ椅子", "ベンチ", "座椅子"}
	var categoryNames = map[int]string{1: "ソファー", 2: "一人掛けソファー", 3: "二人掛けソファー", 4: "コーナーソファー", 5: "二段ソファー", 6: "ソファーベッド", 10: "家庭用チェア", 11: "スツール", 12: "クッションスツール", 13: "ダイニングチェア", 14: "リビングチェア", 15: "カウンターチェア", 20: "キッズチェア", 21: "学習チェア", 22: "ベビーソファ", 23: "キッズハイチェア", 24: "テーブルチェア", 30: "オフィスチェア", 31: "デスクチェア", 32: "ビジネスチェア", 33: "回転チェア", 34: "リクライニングチェア", 35: "投擲用椅子", 40: "折りたたみ椅子", 41: "パイプ椅子", 42: "木製折りたたみ椅子", 43: "キッチンチェア", 44: "アウトドアチェア", 45: "作業椅子", 50: "ベンチ", 51: "一人掛けベンチ", 52: "二人掛けベンチ", 53: "アウトドア用ベンチ", 54: "収納付きベンチ", 55: "背もたれ付きベンチ", 56: "ベンチマーク", 60: "座椅子", 61: "和風座椅子", 62: "高座椅子", 63: "ゲーミング座椅子", 64: "ロッキングチェア", 65: "座布団", 66: "空気椅子"}
	//var categoryIds = [][]int{{2, 3, 4, 5, 6}, {11, 12, 13, 14, 15}, {21, 22, 23, 24}, {31, 32, 33, 34, 35}, {41, 42, 43, 44, 45}, {51, 52, 53, 54, 55, 56}, {61, 62, 63, 64, 65, 66}}

	categoryName := categoryNames[categoryID]
	rootID := rootIds[categoryID]
	rootCategoryName := rootCategoryNames[rootID/10]
	category = Category{categoryID, rootID, categoryName, rootCategoryName}
	return category, err
}

func getCategoryByID(q sqlx.Queryer, categoryID int) (category Category, err error) {
	return getCategoryByIDNew(q, categoryID)
...
func getNewCategryItemsNew(w http.ResponseWriter, r *http.Request) {
	var rootIds = map[int]int{1: 1, 2: 1, 3: 1, 4: 1, 5: 1, 6: 1, 10: 10, 11: 10, 12: 10, 13: 10, 14: 10, 15: 10, 20: 20, 21: 20, 22: 20, 23: 20, 24: 20, 30: 30, 31: 30, 32: 30, 33: 30, 34: 30, 35: 30, 40: 40, 41: 40, 42: 40, 43: 40, 44: 40, 45: 40, 50: 50, 51: 50, 52: 50, 53: 50, 54: 50, 55: 50, 56: 50, 60: 60, 61: 60, 62: 60, 63: 60, 64: 60, 65: 60, 66: 60}
	var rootCategoryName = []string{"ソファー", "家庭用チェア", "キッズチェア", "オフィスチェア", "折りたたみ椅子", "ベンチ", "座椅子"}
	var categoryIds = [][]int{{2, 3, 4, 5, 6}, {11, 12, 13, 14, 15}, {21, 22, 23, 24}, {31, 32, 33, 34, 35}, {41, 42, 43, 44, 45}, {51, 52, 53, 54, 55, 56}, {61, 62, 63, 64, 65, 66}}
	...
	rootID := rootIds[rootCategoryID]
	ret := categoryIds[rootID/10]
	...
	if itemID > 0 && createdAt > 0 {
		...
		inQuery, inArgs, err = sqlx.In(`
			SELECT *
			FROM (SELECT * FROM items WHERE status IN (?, ?)
				AND category_id IN (?)
				AND (created_at < ? OR (created_at <= ? AND id < ?))
				ORDER BY created_at DESC, id DESC
				LIMIT ?) as a
			INNER JOIN
				(SELECT id as user_id, account_name, num_sell_items
					FROM users) as b
		...
		inQuery, inArgs, err = sqlx.In(`
			...
			FROM (SELECT * FROM items WHERE status IN (?,?)
				AND category_id IN (?)
				ORDER BY created_at DESC, id DESC LIMIT ?) as a
			INNER JOIN
			(SELECT id as user_id, account_name, num_sell_items
				FROM users) as b
			ON a.seller_id = b.user_id`,
	...
	for _, item := range items {
		...
		itemSimples = append(itemSimples, ItemSimple{
			ID:         item.ID,
			SellerID:   item.SellerID,
			Seller:     &UserSimple{item.UserID, item.AccountName, item.NumSellItems},
			Status:     item.Status,
			Name:       item.Name,
			Price:      item.Price,
			ImageURL:   getImageURL(item.ImageName),
	...
	rni := resNewItems{
		...
		RootCategoryName: rootCategoryName[rootID/10],
		Items:            itemSimples,
		HasNext:          hasNext,
	}
...
func getItem(w http.ResponseWriter, r *http.Request) {
	...
	err = dbx.Get(&item, `
		SELECT *
		FROM
			(SELECT * FROM items WHERE id = ?) as a
		INNER JOIN (
			SELECT id as user_id, account_name, num_sell_items FROM users) as b
		ON a.seller_id = b.user_id`, itemID)
	if err == sql.ErrNoRows {
